        ...
        <NextIntlClientProvider messages={pick(messages, 'Footer')}>
          <div>
            ...
            <div className='grid gap-x-5 gap-y-1 grid-cols-2 pt-2 sm:pt-4'>
              <NavigationLink className="w-24 sm:w-fit" href={'/'}>{tNav('home')}</NavigationLink>
              <NavigationLink className="w-24 sm:w-fit" href={'/interpretariat'}>{tNav('interpretariat')}</NavigationLink>
              <NavigationLink className="w-24 sm:w-fit" href={'/who_am_i'}>{tNav('whoAmI')}</NavigationLink>
              <a className='w-24 sm:w-fit inline-block transition-all duration-300 ease-in-out hover:scale-90 transform-origin-center' href="https://prosae.odoo.com">{tNav('blog')}</a>
              <NavigationLink className="w-24 sm:w-fit" href={'/biography'}>{tNav('biography')}</NavigationLink>
              <NavigationLink className="w-24 sm:w-fit" href={'/contact'}>{tNav('contact')}</NavigationLink>
              <NavigationLink className="w-24 sm:w-fit" href={'/correction'}>{tNav('correction')}</NavigationLink>
            </div>
          </div>
        </NextIntlClientProvider>
